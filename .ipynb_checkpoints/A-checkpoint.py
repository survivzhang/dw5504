...
# 筛选出含有 "Road User" 的规则（无论在 antecedents 还是 consequents 中）
road_user_rules = rules[
    rules['antecedents'].apply(lambda x: any('Road User' in item for item in x)) |
    rules['consequents'].apply(lambda x: any('Road User' in item for item in x))
]

# 输出查看结果
print(road_user_rules)

# 保存为 CSV
road_user_rules.to_csv('road_user_association_rules.csv', index=False)
